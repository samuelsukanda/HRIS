...
export async function loadHrisData(): Promise<HrisData> {
  ...
  const employees: Employee[] = employeesR.rows.map((r) => ({
    ...
    birthPlace: r.birth_place, birthDate: iso(r.birth_date)!,
    address: r.address, phone: r.phone, email: r.email, joinDate: iso(r.join_date)!,
  ...
  return {
    ...
    roster: rosterR.rows.map((r): RosterEntry => ({
      employeeId: r.employee_id, date: iso(r.date)!.slice(0, 10), shiftId: r.shift_id, id: `${r.employee_id}-${iso(r.date)!.slice(0,10)}`,
    })),
    attendance,
    ...
    announcements: annsR.rows.map((r): Announcement => ({
      id: r.id, title: r.title, body: r.body, category: r.category, date: iso(r.date)!.slice(0, 10),
    })),
...
export async function nextId(table: string, prefix: string): Promise<string> {
  ...
  if (r.rows.length === 0) return `${prefix}-001`;
  const lastId = r.rows[0].id;
  const numStr = lastId.includes("-") ? lastId.split("-")[1] : lastId.slice(prefix.length);
